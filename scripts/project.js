...
function getPictures(projectName) {
    ...
    for(;;i++){
        if(UrlExists('http://res.cloudinary.com/alkaproje/image/upload/projects/' + projectName + '/' + i + '.jpg')){
            $('#' + projectName + ' .pictures').append('<img class="picture" id="resim' + i + '" src="http://res.cloudinary.com/alkaproje/image/upload/projects/' + projectName + '/' + i + '.jpg">');
        }
        else{
            return;
        }
