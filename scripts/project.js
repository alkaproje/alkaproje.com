...
function getPictures(projectName) {
    ...
    for(;;i++){
        if(UrlExists('http://res.cloudinary.com/alkaproje/image/upload/projects/' + projectName + '/' + i + '.jpg')){
            $('#' + projectName + ' .pictures').append('<img class="picture" id="image' + i + '" src="http://res.cloudinary.com/alkaproje/image/upload/c_limit,q_auto:good,w_1000/projects/' + projectName + '/' + i + '.jpg">');
        }
        else{
            return;
        }
